        ...
        <tbody>
          {sortedTareas.length === 0 ? (
            ...
            <tr key={t.id_actividad}>
              ...
              <td>
                {t.estado !== 'Completada' ? (
                  <button type="button" onClick={() => handleCompletarTarea(t.id_actividad)} style={{ background: '#2ecc71', color: 'white', border: 'none', padding: '8px 15px', borderRadius: '4px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }}>
                    <FiCheck /> Listo
                  </button>
                ) : (
