      ...
      {filtered.length > 0 && (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            <button type="button" className="btn btn-secondary btn-sm" onClick={() => setPagina(p => Math.max(1, p - 1))} disabled={pagina === 1}>
              <FiChevronLeft /> Anterior
            </button>
            <span style={{ fontSize: '13px', fontWeight: '600', color: '#0F172A' }}>
              Página {pagina} de {totalPages}
            </span>
            <button type="button" className="btn btn-secondary btn-sm" onClick={() => setPagina(p => Math.min(totalPages, p + 1))} disabled={pagina === totalPages}>
